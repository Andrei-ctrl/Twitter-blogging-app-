...
class Twitter {
                    ...
                    ${
                      img ?
                        `<figure class="tweet-post__image">
												  <img src="${img}" alt="Picture from the post${text}">
                        </figure>` :
                        ''
                      }
                  </div>
  ...
  addTweet({text, img, submit}){
    ...
    console.log(textElem, imgElem, submitElem)
    ...
    imgElem.addEventListener('click', () => {
      imgURL = prompt("Enter image adress")
    })
...
class Post {
  ...
  correctDate(date) {
    if (isNaN(Date.parse(date))){
      ...
      date = date.replaceAll('.', '/') // or date.replace(/\./g, '/')
      console.log(date)
    }
...
/* 
  prevent page from reloading while clicking on home button

  document
        .querySelector('.header__link_home')
        .addEventListener('click', (event) => {
          event.preventDefault();
          console.log('Yey home')
        }) */ 
